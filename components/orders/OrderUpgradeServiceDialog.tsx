...
import { getUpgradableServicesForCompleted, upgradeCompletedOrderService } from "@/app/dashboard/orders/actions"
import type { Order } from "@/lib/features/orders"
import { formatCurrency, getServiceTitle } from "@/lib/features/orders"

interface OrderUpgradeServiceDialogProps {
  open: boolean
...
export function OrderUpgradeServiceDialog({
  open,
  onOpenChange,
  order,
  onSuccess
}: OrderUpgradeServiceDialogProps) {
  ...
  const loadUpgradableServices = async () => {
    ...
    setLoading(true)
    const result = await getUpgradableServicesForCompleted(order.id)
